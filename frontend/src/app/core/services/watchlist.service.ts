...
export class WatchlistService {
  ...
  constructor() {
    ...
    if (storedWatchlistId) {
      this.getWatchlist(parseInt(storedWatchlistId, 10)).subscribe(
        watchlist => this.setActiveWatchlist(watchlist)
      );
    }
  }
  ...
  refreshWatchlists(): Observable<Watchlist[]> {
    return this.http.get<{ watchlists: Watchlist[] }>(`${this.apiUrl}/watchlists`).pipe(
  ...
  addChannelToWatchlist(watchlistId: number, channelId: string): Observable<void> {
    return this.http.post<void>(`${this.apiUrl}/watchlists/${watchlistId}/channels`, { channel_id: channelId }).pipe(
      tap(() => {
  ...
  removeChannelFromWatchlist(watchlistId: number, channelId: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/watchlists/${watchlistId}/channels/${channelId}`).pipe(
      tap(() => {
  ...
  private refreshWatchlistChannels(watchlistId: number): void {
    this.http.get<{ channels: Channel[] }>(`${this.apiUrl}/watchlists/${watchlistId}/channels`).pipe(
